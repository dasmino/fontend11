...
$(document).ready(function() {
	...
	.done(function(data) {
		$( "#loadMediaModelfavicon" ).append(data.html);
	});
});
...
$('body').on('click', '.choose_img', function(event) {
	...
	body_img.find('.img-active').each(function(){
		...
		if (list.length == 0) {
			...
			$.ajax({
				url: baseUrlcloud+'index.php?mod=settings&controller=settings&action=updateInfo',
				type: 'POST',
				dataType: 'json',
...
$('body').on('dblclick', '#myModalFavicon .media-col img.img-folder-media', function(event) {
	...
	var directory = $('#loadMediaModelfavicon #directory').val();
	$.ajax({
...
$('body').on('dblclick', '#myModalLogo .media-col img.img-folder-media', function(event) {
	event.preventDefault();
	var check_folder = $(this).parent('.media-col').attr('data-folder');
	var directory = $('#loadMediaModel #directory').val();
	$.ajax({
		url: baseUrlcloud+'settings/settings/openDirectory',
		type: 'POST',
		dataType: 'json',
...
$('body').on('dblclick','#myModalFavicon #back_folder', function(event) {
	event.preventDefault();
...
$('body').on('dblclick','#myModalLogo #back_folder', function(event) {
	event.preventDefault();
	var current_folder = $(this).parent('.modal-body');
	var directory = $('#loadMediaModel #directory').val();
	$.ajax({
