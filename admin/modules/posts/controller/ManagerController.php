...
class ManagerController extends Controller{
	...
	public function __construct(){
		parent::__construct();
		$this->modelCategory = $this->loadModel('Categories');
		$this->modelProduct = $this->loadModel('posts');
		$this->modelPost = $this->loadModel('posts');
	...
	public function sortablecon(){
		if (isset($_GET['listItemcon'])) {
			$manager = $this->modelManager->getDataById($_GET['id']);
			$manager_content = json_decode($manager['content'],true);
			foreach ($_GET['listItemcon'] as $key => $value) {
				foreach ($manager_content as $k => $v) {
					if($value == $v['id']){
						$_GET['listItemcon'][$key] = $manager_content[$k];
					}
				}
			}
			$remove_values_arr = remove_duplicate_values_array($_GET['listItemcon']);
			$data = array(
					'content'	=> json_encode($remove_values_arr)
				);
			$this->modelManager->update($data,$_GET['id']);
			$data_mess = array(
				'status'	=> true,
				'mess'		=> lang('update_page_success')
			);
	...
	public function dellAll(){
		if (isset($_POST['all'])) {
			if (!empty($_POST['all']) &&  is_array($_POST['all'])) {
				...
				$data_mess = array(
					'status'	=> true,
					'redirect'		=> base_url().'posts/manager/index'
				);
				echo json_encode($data_mess);
            }
		}
	...
	public function getPostByCate(){
		if (isset($_POST['id']) && is_numeric($_POST['id'])) {
			$id = $_POST['id'];
			$data = $this->modelManager->getPostByCateModel(0, 20,$id,null,$where_search = null);
			$data_mess = $this->modelManager->check_load_more_postByCate($number = 20, $ajax = false, $id,null);
			$html = '';
			if (!empty($data)) {
				foreach ($data as $key => $value) {
					$html .= '<div class="col-md-12" data-id="'.$value['id'].'">
								<div class="imgs">
		                    		<img src="'.base_url_cloud().'timthumb.php?src='.base_url_cloud().'cdn/'.$value['thumbnail'].'&h=100&w=100&zc=1" alt="'.$value['title'].'" class="img-thumbnail">
		                    		<h5 class="text-center">'.stripslashes($value['title']).'</h5>
	...
	public function ajaxLoadMore(){
		if (!empty($_POST['data'])) {
			...
			$data = $this->modelManager->getPostByCateModel($start, 20, $cate_id, $arrid, null);

			$data_mess = $this->modelManager->check_load_more_postByCate($start + 20, $ajax = false, $cate_id,$arrid);
			$html = '';
			if (!empty($data)) {
				foreach ($data as $key => $value) {
					$html .= '<div class="col-md-12" data-id="'.$value['id'].'">
	...
	public function ajaxAddProductHome(){
		if (!empty($_POST['data']) && isset($_POST['id_cate'])) {
			...
			foreach ($_POST['data'] as $value) {
				$result[] = $this->modelManager->getDataByIdpost($value);
			}
			...
			if ($remove_values_arr!='') {
				...
				foreach ($remove_values_arr as $key => $value) {
					$html .= '<div class="col-md-12" data-id="'.$value['id'].'" data-key="'.$key.'">
									<div class="imgs">
			                    		<img src="'.base_url_cloud().'timthumb.php?src='.base_url_cloud().'cdn/'.$value['thumbnail'].'&h=100&w=100&zc=1" alt="'.$value['title'].'" class="img-thumbnail">
			                    		<h5 class="text-center">'.stripslashes($value['title']).'</h5>
			                    	</div>
