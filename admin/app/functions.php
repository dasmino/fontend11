...
if (!function_exists('listCategories')){
    ...
    foreach($data as $k=>$value){
        if($value['parent_id'] == $parent){
            $id=$value['id'];
            echo "<li><a data-title='".$value['title']."' data-alias='".$value['alias']."' data-id='".$value['id']."' href='#' data-link='".base_url_fontend().$value['alias'].'-c'.$value['id'].'.htm'."'>".$value['title']."</a></li>";
            unset($data[$k]);
...
if (!function_exists('listCategoriesController')){
    function listCategoriesController($data, $check=null , $view='posts' , $parent=0 , $text="-" , $select=0){
        ...
        if($check!=null){
            ...
            if(isset($ok) && $ok == 1){
                if ($parent==0) {
                    echo '<ul class="list-categories" data-type="'.lang('categories').'">';
                }else{
                    echo '<ul>';
                }
                ...
                foreach($data as $k=>$value){
                    if($value['parent_id'] == $parent){
                        $id=$value['id'];
                        echo '<li><div class="checker"><span ';
                        if (preg_match("/,". $value['id'].",/",$check, $matches)){
                            echo "class='after_opacity'";
                        }
                        echo '><input type="checkbox" class="styled" name="categories[]" value="'.$value['id'].'" ';
                        if (preg_match("/,". $value['id'].",/",$check, $matches)){
                            echo "checked";
                        }
                        echo ' ></span></div><label for="category-item-'.$value['alias'].'">'.$value['title'].'</label>';
                        unset($data[$k]);
                        listCategoriesController($data,$check,$view,$id,$text."--",$select);
                        echo '</li>';
                    }
                }
                echo '</ul>';
            ...
            foreach($data as $k=>$value){
                if($value['parent_id'] == $parent){
                    $id=$value['id'];
                    echo '<li>
                    <div class="checker"><span><input type="checkbox" class="styled" name="categories[]" value="'.$value['id'].'"></span></div>
                    <label for="category-item-'.$value['alias'].'">'.$value['title'].'</label>
                </li>';
                unset($data[$k]);
                listCategoriesController($data,$check,$view,$id,$text."--",$select);
            }
...
if (!function_exists('CallPermission')){
    function CallPermission($data,$parent=0,$text="-",$select=0){
        foreach($data as $k=>$value){
            if($value['parent_id'] == $parent){
                $id=$value['id'];
                if($select != 0 && $id == $select){
                    echo "<option value='$value[id]' selected='selected'>".$text." ".$value['name']."</option>";
                }else{
                    echo "<option value='$value[id]'>".$text." ".$value['name']."</option>";
                }
                unset($data[$k]);
                CallPermission($data,$id,$text."--||",$select);
            }
        }
    }
}
// function so sanh 1 chuoi vs 1 mang khong can $key 
