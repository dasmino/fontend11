...
if (!function_exists('recursiveMenuContent')){
        function recursiveMenuContent($data,$parent=0,$count=0){
            ...
            if (isset($data[$parent])) {
                ...
                if($parent==0){
                    echo "<li><a href='".$_web['settings']['link_facebook']."' target='_blank'><img src='http://travelchina.co.il/wp-content/uploads/2016/02/icon_facebook.png' style='cursor:pointer;height:20px' scale='0'></a></li>";
                }
                echo "</ul>";
            }
        }
...
if (!function_exists('recursiveMenu1')){
        function recursiveMenu1($data,$parent=0,$count=0){
            global $_web;
            if (isset($data[$parent])) {
                if ($parent==0) {
                    // echo "<div>";
                }else{
                    echo "<div class='list-footer'><ul>";
                }
                foreach ($data[$parent] as $k=>$value) {
                        $id=$value['id'];
                        if (isset($data[$id])) {
                            if ($count>0) {
                                $class = 'class="list-footer"';
                            }else{
                                $class='class="uk-width-medium-2-10"';
                            }
                            echo "<div $class>";
                        }else{
                            echo "<div>";
                        ...
                        if (isset($data[$id])) { 
                            ...
                            echo "<div class='title-footer'>".$value['title']."</div>";
                        }else{
                            echo "<li><a href='".$value['link']."'>".$value['title']."</a></li>";
                        }
                        unset($data[$k]);
                        recursiveMenu1($data,$id,$count+1);
                    echo "</div>";
                }
                echo "</ul></div>";
            }
           ...
           foreach ($cdir as $key => $value){ 
              if (!in_array($value,array(".",".."))){ 
                 ...
                 { 
                    ...
                    $allfile = dirToArray($dir . DIRECTORY_SEPARATOR . $value);

                    $result[$n]['allfile'] = $allfile[0];
